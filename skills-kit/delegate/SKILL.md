...
overwritten · on schema-3 Core/Standard, `scripts/check-banka-thresholds.sh`,
consulted before archiving and re-run after appending any ticket (Protocol
Section 2.9); schema-2 pre-migration Core/Standard predates the script
...
`context/delegation-queue.md`. Create the resolved queue only for an active
schema. Append tickets — never silently overwrite a queue that already has
