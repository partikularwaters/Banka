...
**Before installing over an existing pre-rename install:** check `~/.claude/commands/` (the old single-file command location) and `~/.claude/skills/` for stale files under the old CTP names (`architect`, `review`, `recover`, `imprint`, plus any old `remember`/`scale` predating this package) — remove those first. A same-named old command shadowing or conflicting with a new skill (this bit `remember` and `scale` specifically, since those two names carried over unchanged) is worse than a missing skill.
